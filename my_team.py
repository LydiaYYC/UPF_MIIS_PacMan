...
class RunningPacmanCaptureAgent(CaptureAgent):
    ...
    def choose_action(self, game_state):
        ...
        #If there are more than 3 food in the game, move forward to the closest 3 food
        if len(food_list) > 3:
            nearest_foods = sorted(food_list, key=lambda f: self.get_maze_distance(my_pos, f))[:3]
        else:
            nearest_foods = food_list
        ...
        #uesing A* to find the closest 3 or 1 food
        for food in nearest_foods:
            path = self.a_star_search(game_state, my_pos, food)
...
class Defensive(RunningPacmanCaptureAgent):
    ...
    def get_features(self, game_state, action):
        ...
        # to make sure teh defensive ghost won't exceed the bounds and turn into pacman.
        mid_x = (game_state.data.layout.width // 2) -2 if self.red else (game_state.data.layout.width // 2) +1
        if (self.red and my_pos[0] > mid_x) or (not self.red and my_pos[0] < mid_x):
            features["out_of_bounds"] = 1
